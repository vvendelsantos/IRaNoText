...
st.markdown("""
    <div style='margin-bottom: 2rem;'>
        <p style='color: #666;'>Ferramenta profissional para detecção de siglas, palavras compostas e geração de corpus textual compatível com IRaMuTeQ</p>
    </div>
""", unsafe_allow_html=True)

tabs = st.tabs(["📝 Pré-análise de Texto", "📑 Geração de Corpus"])

with tabs[0]:
    # ========================== PARTE 1 - PRÉ-ANÁLISE ==========================
    with st.container():
        st.header("🔍 Detecção de Elementos Textuais")
        st.markdown("Analise seu texto para identificar siglas e palavras compostas automaticamente.")
        ...
        with col1:
            if st.button("Analisar Texto", type="primary", use_container_width=True):
                if texto_input.strip():
                    ...
                    with col_res1:
                        with st.expander(f"🧩 Palavras Compostas ({len(compostas)})", expanded=True):
                            if compostas:
                    ...
                    with col_res2:
                        with st.expander(f"🧾 Siglas Detectadas ({len(siglas)})", expanded=True):
                            if siglas:
                                for sigla in siglas:
                                    st.markdown(f"- {sigla}")
                            else:
...
with tabs[1]:
    ...
    with st.container():
        st.header("📂 Geração de Corpus Textual")
        st.markdown("""
            <div style='margin-bottom: 1.5rem;'>
                <p>Transforme seus dados textuais em um corpus formatado para análise no IRaMuTeQ.</p>
            </div>
        """, unsafe_allow_html=True)

        with st.expander("ℹ️ Instruções Detalhadas", expanded=True):
            st.markdown("""
            ### Estrutura Requerida da Planilha

            Sua planilha Excel (**.xlsx**) deve conter **três abas** com os seguintes nomes:
        ...
        with col_dl2:
            with open("textos_selecionados.xlsx", "rb") as textos:
                st.download_button(
                    label="📥 Exemplo com Textos para Análise",
                    data=textos,
        ...
        file = st.file_uploader(
            "Carregue seu arquivo Excel preenchido",
            type=["xlsx"],
        ...
        def converter_numeros_por_extenso(texto):
            ...
            for palabra in palabras:
                ...
                if palabra_lower in unidades:
                    resultado.append(str(unidades[palabra_lower]))
                elif palabra_lower in dezenas:
                    resultado.append(str(dezenas[palabra_lower]))
                elif palabra_lower in centenas:
                    resultado.append(str(centenas[palabra_lower]))
                elif palabra_lower in multiplicadores:
                    resultado.append(str(multiplicadores[palabra_lower]))
                else:
                    resultado.append(processar_palavra(palabra))
            ...
            try:
                ...
                if st.button("Gerar Corpus", type="primary", use_container_width=True):
                    ...
                    if corpus.strip():
                        ...
                        col_prev, col_stats = st.columns([2, 1])
                        
                        with col_prev:
                            st.subheader("Pré-visualização do Corpus")
                            st.code(corpus[:2000] + ("..." if len(corpus) > 2000 else ""), language="text")
                            
                            buf = io.BytesIO()
                            buf.write(corpus.encode("utf-8"))
                            st.download_button(
                                "Baixar Corpus Completo",
                                data=buf.getvalue(),
                        ...
                        with col_stats:
                            ...
                            st.text_area("Métricas do processamento", estatisticas, height=200)
                    else:
