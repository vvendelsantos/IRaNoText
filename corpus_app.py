...
def detectar_palavras_compostas(texto):
    stopwords = {'a', 'o', 'de', 'da', 'das', 'do', 'dos', 'em', 'na', 'nas', 'no', 'nos', 'para', 'por', 'com', 'sem', 'sobre'}
    palavras = texto.lower().split()
    palavras_compostas_auto = []

    for i in range(len(palavras) - 1):
        if palavras[i] not in stopwords and palavras[i + 1] not in stopwords:
            composta = f"{palavras[i]} {palavras[i + 1]}"
            if composta not in palavras_compostas_auto:
                palavras_compostas_auto.append(composta)
    
    return palavras_compostas_auto

# Função para detectar siglas
def detectar_siglas(texto):
    siglas = re.findall(r'\b([A-Z]{2,})\b', texto)
    siglas = list(set(siglas))  # Remover siglas duplicadas
    return siglas
